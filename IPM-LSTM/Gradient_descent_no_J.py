...
def main():
    ...
    J, F, J1 = problem.cal_kkt(x, eta, s, lamb, zl, zu, mu=0.0, sigma=args.sigma, lambs = lambda_hessian, newton_flag = False)
    ...
    while mu_k > 1e-8 and res_norm > args.tol:
        ...
        J, F, J1 = problem.cal_kkt(x, eta, s, lamb, zl, zu, 0, args.sigma, lambda_hessian, newton_flag = False)
        res_norm = F.norm().item()
        iter_idx = -1
        while res_norm > eps_mu:
            ...
            J_aff, F_aff, J1 = problem.cal_kkt(x, eta, s, lamb, zl, zu, mu_k, 0, lambda_hessian, newton_flag = False)
            ...
            # 3) Corrector solve
            J, F, _ = problem.cal_kkt(x, eta, s, lamb, zl, zu, mu_k, sigma_k, lambda_hessian, newton_flag = False)
            delta = -F
            ...
            J, F, J1 = problem.cal_kkt(x, eta, s, lamb, zl, zu, mu_k, sigma_k, lambda_hessian, newton_flag = False)
            res_norm  = F.norm().item()
