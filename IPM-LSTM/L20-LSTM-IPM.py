# import os
# os.environ.setdefault("PYCUTEST_CACHE", os.path.expanduser("~/.cache/pycutest"))
# os.environ.setdefault("ARCHDEFS", os.path.expanduser("~/cutest/archdefs"))
# os.environ.setdefault("SIFDECODE", os.path.expanduser("~/cutest/sifdecode/bin/sifdecode"))
# os.environ.setdefault("MASTSIF",  os.path.expanduser("~/cutest/maSTSIF"))
...
from problems.QP_box_cutest import Box_Constraints
from problems.QP import BoxQP
...
LR_META  = 1e-5                   # learning-rate for Adam
...
def project_tube_all(
    ...
    mask_lb = problem.has_lb         # [B,n] – where a lower bound exists
    mask_ub = problem.has_ub            # [B,n] – where an upper bound exists
...
def solve_one_qcqp(meta_opt, problem: "Box_Constraints",
                   net: PS_L20_LSTM,
                   muP0=1e-4, muB0=1,
                   max_outer=MAX_OUTER, train_test = 'train', val_stop = 5, print_level = 3, problem_type = 'QP'):
    ...
    for outer in range(1, max_outer+1):
        ...
        mask_lb = problem.has_lb 
        mask_ub = problem.has_ub
...
if __name__ == "__main__":
    ...
    # mat_names = [
    #     "unconstrained_QP_nonconvex_full_Q_dim_1000"
    # ]

    mat_names = ["unconstrained_QP_nonconvex_full_Q_dim_10_lbubrandom"]
    ...
    for fp in file_paths:
        pools.append({
            "train": Box_Constraints(source = 'qp', prob_type='QP_unconstrained', learning_type='train', file_path=fp, seed=args.seed),
            "val":   Box_Constraints(source = 'qp', prob_type='QP_unconstrained', learning_type='val',   file_path=fp, seed=args.seed),
            "test":  Box_Constraints(source = 'qp', prob_type='QP_unconstrained', learning_type='test',  file_path=fp, seed=args.seed),
        })
    
    with open("probs/BCset.txt", "r") as f:
        cutest_names = [ln.strip() for ln in f if ln.strip() and not ln.strip().startswith("#")]

    pools_cutest = []
    for nm in cutest_names:
        pools_cutest.append({
            "test": Box_Constraints(
                source = 'cutest',        # your dataset class should route to Problem(source="cutest", name=nm)
                name=nm              # pass the CUTEst problem name
            )
        })
    ...
    #net = PS_L20_LSTM(problem=pools_cutest[0]["test"], K_inner=K_INNER, device=DEVICE).to(DEVICE)
    ...
    # test evaluation 
        ...
        net.to(DEVICE).eval()
        with torch.no_grad():
            print(len(pools_cutest))
            for ds_idx, pack in enumerate(pools_cutest[1:2]):
                print(f"\nTesting: {mat_names[ds_idx]}")
                ds_start = time.perf_counter()
                _ = solve_one_qcqp(optim.Adam(net.parameters(), lr=LR_META), pack["test"], net, train_test='test', problem_type = 'cutest')
                ...
                sols, _, para_times, iters = pack["test"].opt_solve(solver_type='ipopt_cutest', tol=1e-2)
