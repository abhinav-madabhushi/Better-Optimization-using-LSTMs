...
class QP(object):
    ...
    """
    def __init__(self, prob_type, learning_type, val_frac=0.0000, test_frac=0.01, device='cuda' if torch.cuda.is_available() else 'cpu', seed=17, **kwargs):
        super().__init__()
    ...
    def dual_feasibility(self, x, x1, x2, s,       # add s1 and s2 if there are bounds for inequality
        ...
        if self.num_eq != 0:
            ...
            At = self.A.t().unsqueeze(0).expand(B, -1, -1)
        ...
        mask_lb = torch.isfinite(lb_vals).to(device)     # [B,n,1]
        mask_ub = torch.isfinite(ub_vals).to(device)
