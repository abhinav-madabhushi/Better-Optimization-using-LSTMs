...
class PS_L20_LSTM(nn.Module):
    # ------------------------------------------------------------
    def __init__(self, problem, hidden_size=128, num_layers=5, K_inner = 1, device='cuda' if torch.cuda.is_available() else 'cpu', fixedmuNumber = 15): 
        super().__init__()
        # self.input_size = 0
        # self.output_size = 0
        ...
        #     self.input_size = self.input_size + (10*var)
        ...
        self.feat_dim = 6          # adjust if you change the feature set below
        self.hidden_size = hidden_size
        self.w = 2                 # band width for L (diag + w sub-diagonals); tune as you like

        self.lstm = nn.LSTM(
            input_size  = self.feat_dim,   # <-- constant, not tied to n
            hidden_size = hidden_size,
            num_layers  = num_layers,
            batch_first = True,
        )

        # per-token head: predicts (w+1) numbers for each coordinate: diag + w sub-diagonals
        self.row_head = nn.Linear(hidden_size, self.w + 1)
    ...
    def forward(self,
        ...
        for i in range (1, self.K_inner + 1):
            ...
            #     N, N, N, N  # b, a, b1, b2
            # )
            # p, b, a, b1, b2 = torch.split(out, sizes, dim=1)
            ...
            m = 3 * n
            if n_vec is None:
                mask_x  = x.new_ones(B, n)
            else:
                n_vec_b = n_vec.reshape(-1).long().to(x.device)
                ar = torch.arange(n, device=x.device).view(1, n).expand(B, n)
                mask_x = (ar < n_vec_b.view(B, 1)).float()
            mask_z1 = mask_x
            mask_z2 = mask_x
            mask_v  = torch.cat([mask_x, mask_z1, mask_z2], dim=1)            # [B, m]
            lengths = mask_v.sum(dim=1).long()                                # [B]

            # ----- collect per-token features (all [B,m,1]) -----
            # values
            val_tok = torch.cat([x, z1, z2], dim=1).unsqueeze(-1)             # [B,m,1]

            # gradients (assumes you already computed grad_x, grad_z1, grad_z2 as [B,n])
            grad_tok = torch.cat([grad_x, grad_z1, grad_z2], dim=1).unsqueeze(-1)

            # estimates (masked like you already do elsewhere)
            x1E = x1E * mask_x; x2E = x2E * mask_x; z1E = z1E * mask_z1; z2E = z2E * mask_z2
            est_tok = torch.cat([x1E, z1E, z2E], dim=1).unsqueeze(-1)         # [B, m, 1] (use your choice)

            # muB broadcast
            muB_tok = muB.view(B, 1, 1).expand(-1, m, -1)                     # [B, m, 1]

            # bound flags aligned to x-block and repeated (if you have finite lb/ub)
            if problem.num_lb != 0 and problem.num_ub != 0:
                lb_f = torch.isfinite(problem.lb.expand(B, n)).float()
                ub_f = torch.isfinite(problem.ub.expand(B, n)).float()
            else:
                lb_f = x.new_zeros(B, n)
                ub_f = x.new_zeros(B, n)
            flag_tok = torch.cat([lb_f, lb_f, lb_f], dim=1).unsqueeze(-1)     # [B, m, 1]
            flag2_tok= torch.cat([ub_f, ub_f, ub_f], dim=1).unsqueeze(-1)

            # ----- stack features into [B, m, feat_dim] and mask -----
            Xseq = torch.cat([val_tok, grad_tok, est_tok, muB_tok, flag_tok, flag2_tok], dim=-1)  # [B,m,feat_dim=6]
            Xseq = Xseq * mask_v.unsqueeze(-1)

            # ----- run LSTM over variable sequence length -----
            from torch.nn.utils.rnn import pack_padded_sequence, pad_packed_sequence
            packed = pack_padded_sequence(Xseq, lengths.cpu(), batch_first=True, enforce_sorted=False)
            Y_packed, (h_new, c_new) = self.lstm(packed, states)
            Y, _ = pad_packed_sequence(
                Y_packed, batch_first=True, total_length=mask_v.size(1)
                )
            Y = Y * mask_v.unsqueeze(-1)           # [B, m_max, H]

            # ----- per-token row params: [B, m_max, w+1] -----
            row_params = self.row_head(Y)
            diag = F.softplus(row_params[..., 0]) + 1e-8                     # [B, m_max]
            subs = row_params[..., 1:]                                       # [B, m_max, w]

            # ----- build banded lower-triangular L for each batch -----
            B, m_max = diag.shape
            L = diag.new_zeros(B, m_max, m_max)
            idx = torch.arange(m_max, device=diag.device)
            L[:, idx, idx] = diag * mask_v                                   # diagonal with mask

            for b in range(1, self.w + 1):
                i = torch.arange(b, m_max, device=diag.device)
                j = i - b
                vals = subs[:, i, b-1]
                L[:, i, j] = vals * (mask_v[:, i] * mask_v[:, j])

            # zero out interactions with padded tokens
            mask_H = mask_v.unsqueeze(2) * mask_v.unsqueeze(1)
            P = (L @ L.transpose(1, 2)) * mask_H

            # descent direction on concatenated [x, z1, z2]
            g = torch.cat([grad_x, grad_z1, grad_z2], dim=1) * mask_v        # [B, m_max]
            d = -(P @ g.unsqueeze(-1)).squeeze(-1) * mask_v

            dx, dz1, dz2 = torch.split(d, (n, n, n), dim=1)
            step = 0.01
            x_new  = (x  + step * dx)  * mask_x
            z1_new = (z1 + step * dz1) * mask_z1
            z2_new = (z2 + step * dz2) * mask_z2
            ...
            if self.device == 'cpu':
                ...
                for i in range(min(5, lb.shape[0])):
                    ...
                    # print(f"  f_val_should_be {i} =  {f_val_should_be[i]}")
                    # print(f"  lb_1_should_be {i} =  {lb_1_should_be[i]}")
                    # print(f"  lb_2_should_be {i} =  {lb_2_should_be[i]}")
                    # print(f"  lb_3_should_be {i} =  {lb_3_should_be[i]}")
                    # print(f"  lb_4_should_be {i} =  {lb_4_should_be[i]}")
                    # print(f"  AL_1_should_be {i} =  {AL_1_should_be[i]}")
                    # print(f"  ub_1_should_be {i} =  {ub_1_should_be[i]}")
                    # print(f"  ub_2_should_be {i} =  {ub_2_should_be[i]}")
                    # print(f"  ub_3_should_be {i} =  {ub_3_should_be[i]}")
                    # print(f"  ub_4_should_be {i} =  {ub_4_should_be[i]}")
                    # print(f"  AL_2_should_be {i} =  {AL_2_should_be[i]}")
                    # for j in range(lb.shape[1]):
                    #     print(f"  x {j} =  {x[i, j]}")
                    #     print(f"  z1 {j} =  {z1[i, j]}")
                    #     print(f"  z2 {j} =  {z2[i, j]}")
