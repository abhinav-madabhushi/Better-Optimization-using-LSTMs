...
def main():
    ...
    parser.add_argument('--ls_c', type=float, default=1e-5, help='Armijo constant for line search')
    ...
    J, F, J1 = problem.cal_kkt_newton(x, eta, s, lamb, zl, zu, mu=0.0, sigma=args.sigma, lambs = lambda_hessian, newton_flag = False)
    ...
    while mu_k > 1e-8 and res_norm > args.tol:
        ...
        J, F, J1 = problem.cal_kkt_newton(x, eta, s, lamb, zl, zu, 0, args.sigma, lambda_hessian, newton_flag = False)
        res_norm = F.norm().item()
        iter_idx = -1
        while res_norm > eps_mu:
            ...
            J_aff, F_aff, J1 = problem.cal_kkt_newton(x, eta, s, lamb, zl, zu, mu_k, 0, lambda_hessian, newton_flag = False)
            ...
            # 3) Corrector solve
            J, F, _ = problem.cal_kkt_newton(x, eta, s, lamb, zl, zu, mu_k, sigma_k, lambda_hessian, newton_flag = False)
            ...
            J, F, J1 = problem.cal_kkt_newton(x, eta, s, lamb, zl, zu, mu_k, sigma_k, lambda_hessian, newton_flag = False)
            res_norm  = F.norm().item()
