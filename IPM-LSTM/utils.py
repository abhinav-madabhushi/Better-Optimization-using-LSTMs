...
def calculate_step(data, x, delta_x, eta, delta_eta, s, delta_s, zl, delta_zl, zu, delta_zu,
                             lb, ub, tau, use_line_search, device='cpu'):
    """
    ...
    # ensure all inputs use [B, *, 1] so bound steps align correctly
    x, delta_x = _col(x), _col(delta_x)
    eta, delta_eta = _col(eta), _col(delta_eta)
    s, delta_s = _col(s), _col(delta_s)
    zl, delta_zl = _col(zl), _col(delta_zl)
    zu, delta_zu = _col(zu), _col(delta_zu)
    lb = _col(lb) if torch.is_tensor(lb) else lb
    ub = _col(ub) if torch.is_tensor(ub) else ub
